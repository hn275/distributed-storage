...
def generate_cl_num_conn_distribution(filename, output_file):
    ...
    for idx, node_id in enumerate(node_info):
        plt.plot(node_info[node_id][time_label], node_info[node_id][conn_label], marker='o', linestyle='-', label=str(node_id), color=colors[idx])
...
def generate_lb_num_conn_distribution(filename, output_file):
    ...
    for line in fd:
        tokens = line.split(",", maxsplit=6) #TODO: may need to change this val depending on csv format
    ...
    for idx, node_id in enumerate(node_to_conn):
        plt.plot(times, node_to_conn[node_id], marker='o', linestyle='-', label=str(node_id), color=colors[idx])
...
def filter_lb_logs(filename, output_file, node_filter = None, time_filter = None):
    ...
    for line in fd_r:
        ...
        if (event == "user-joined" and 
            (node_filter == None or node_filter == node_id) and
            (time_filter == None or time_filter <= timestamp)):
            fd_w.write(line)
...
""")
    ...
    if option == "filter-cl" or option == "filter-lb":
        ...
        if option == "filter-cl":
            filter_cluster_logs(input_file, output_file, node_filter=node, time_filter=time)
        if option == "filter-lb":
        ...
        print("Invalid option given. Must be one of filter-cl, filter-lb, plot-cl, plot-lb")
