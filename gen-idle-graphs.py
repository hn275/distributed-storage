...
def format_csvdata(CSVDATA):
    ...
    for experiment in CSVDATA:
        for node in CSVDATA[experiment]:
            ...
            endTime = int(CSVDATA[experiment][node]["EndDuration"]) + int(
                CSVDATA[experiment][node]["EndTimestamp"]
            )
...
def save_to_dictonary(nodeID, eventType, timeStamp, duration, peer, rate):
    ...
    if ("Node " + str(nodeID)) not in CSVDATA["Request Rate: " + str(rate)]:
        CSVDATA["Request Rate: " + str(rate)]["Node " + str(nodeID)] = {
            "NodeID": nodeID,
            "StartTimestamp": 0,
        ...
        if (peer != "") & (
            CSVDATA["Request Rate: " + str(rate)]["Node " + str(nodeID)]["Peer"] == ""
        ):
...
def create_table(CSVDATA):
    ...
    sortColumns = [
        ...
        + ", "
        + HOMO
    ] + sorted(df.columns[1:], key=lambda x: int(x.split()[1]))
    df = df[sortColumns]
